...
pkgdatadir = $(datadir)/macrocoin
pkgincludedir = $(includedir)/macrocoin
pkglibdir = $(libdir)/macrocoin
pkglibexecdir = $(libexecdir)/macrocoin
...
CONFIG_HEADER = $(top_builddir)/src/config/macrocoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/macrocoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Macrocoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /tmp/Macrocoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Macrocoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Macrocoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = macrocoin-cli
BITCOIN_DAEMON_NAME = macrocoind
BITCOIN_GUI_NAME = macrocoin-qt
BITCOIN_TX_NAME = macrocoin-tx
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Macrocoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = macrocoin
PACKAGE_BUGREPORT = www.macrocoin.org
PACKAGE_NAME = Macrocoin Core
PACKAGE_STRING = Macrocoin Core 1.0.0
PACKAGE_TARNAME = macrocoin
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Macrocoin
abs_srcdir = /tmp/Macrocoin
abs_top_builddir = /tmp/Macrocoin
abs_top_srcdir = /tmp/Macrocoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Macrocoin/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_macrocoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_macrocoin_coverage.info test_macrocoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Macrocoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/macrocoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/macrocoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/macrocoin-config.h
$(top_srcdir)/src/config/macrocoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/macrocoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Macrocoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Macrocoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Macrocoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_macrocoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_macrocoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_macrocoin_filtered.info: test_macrocoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_macrocoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_macrocoin_coverage.info: baseline_filtered_combined.info test_macrocoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_macrocoin_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_macrocoin_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_macrocoin_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_macrocoin.coverage/.dirstamp:  test_macrocoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_macrocoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_macrocoin.coverage/ total.coverage/ $(OSX_APP)
