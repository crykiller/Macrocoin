...
public:
    ...
    bool WriteZMCCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMCCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMCCSeed_deprecated(uint256& seed);
    bool EraseZMCCSeed();
    bool EraseZMCCSeed_deprecated();

    bool WriteZMCCCount(const uint32_t& nCount);
    bool ReadZMCCCount(uint32_t& nCount);
