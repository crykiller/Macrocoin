...
// The zMCC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMCC is a hash of the serial
...
{
    //Create an output returning the zMCC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMCCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMCC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMCC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMCCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMCC output", __func__);
...
{
    CzMCCTracker* zmccTracker = pwallet->zmccTracker.get();
    CMintMeta meta;
    if (!zmccTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmccTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MCC stake is the outpoint
