...
CzMCCWallet::CzMCCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmcc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMCCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMCCSeed_deprecated()) {
                    LogPrintf("%s: Updated zMCC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmcc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmcc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMCCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMCCCount(nCountLastUsed);
    else if (!walletdb.ReadZMCCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMCCWallet::Lock()
{
    seedMaster = 0;
}

void CzMCCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMCCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMCC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMCCWallet::LoadMintPoolFromDB()
{
...
void CzMCCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMCCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMCCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmccTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMCCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMCC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmccTracker which also adds to database
    pwalletMain->zmccTracker->Add(dMint, true);
...
void CzMCCWallet::SeedToZMCC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMCCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMCCWallet::GenerateDeterministicZMCC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMCCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMCCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
