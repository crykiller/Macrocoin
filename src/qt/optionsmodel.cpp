...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MCC);
    ...
    if (!settings.contains("nAnonymizeMacrocoinAmount"))
        settings.setValue("nAnonymizeMacrocoinAmount", 1000);

    nAnonymizeMacrocoinAmount = settings.value("nAnonymizeMacrocoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMacrocoinAmount"))
        SoftSetArg("-anonymizemacrocoinamount", settings.value("nAnonymizeMacrocoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in macrocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMacrocoinAmount:
            return QVariant(nAnonymizeMacrocoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMacrocoinAmount:
            nAnonymizeMacrocoinAmount = value.toInt();
            settings.setValue("nAnonymizeMacrocoinAmount", nAnonymizeMacrocoinAmount);
            emit anonymizeMacrocoinAmountChanged(nAnonymizeMacrocoinAmount);
            break;
        case CoinControlFeatures:
